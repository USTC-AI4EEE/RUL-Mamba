...
parser.add_argument('--model', default='Autoformer',help='Model name.')
...
for start_point in args.start_point_list:
    ...
    while count<args.count:
        ...
        # --------------------------------------step 2: 网络构建--------------------------------------------------------
        # ----------------------------------------- 创建基线模型 ----------------------------------------------
        from ModelsModify.Autoformer import AutoFormerNetModel
        model = AutoFormerNetModel.from_dataset(
            training,
        ...
        best_model = AutoFormerNetModel.load_from_checkpoint(best_model_path).to(device=device)
